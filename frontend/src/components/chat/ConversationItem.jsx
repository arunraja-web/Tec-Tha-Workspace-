  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all cursor-pointer border ${
        isSelected
          ? 'bg-[#2a3942] border-[#005c4b] shadow-xs'
          : 'bg-[#111b21] hover:bg-[#202c33] border-neutral-800/50'
      }`}
    >
      {/* Avatar Container */}
      <div className="relative shrink-0">
        <div
          className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-sm text-white shadow-xs ${
            isGroup
              ? 'bg-gradient-to-br from-[#005c4b] to-[#008069]'
              : 'bg-gradient-to-br from-[#005c4b] to-[#008069]'
          }`}
        >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1 mb-1">
          <h3 className="text-xs sm:text-sm font-bold text-[#e9edef] truncate">
            {title}
          </h3>
          {timeString && (
            <span className="text-[10px] font-semibold text-[#8696a0] shrink-0">
              {timeString}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {unreadCount > 0 && (
            <span className="px-2 py-0.5 rounded-full bg-[#00a884] text-white text-[10px] font-bold shrink-0 shadow-xs">
